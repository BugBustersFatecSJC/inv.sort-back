...
function ProductCategory(props) {
  ...
  const fetchUnits = async () => {
    try {
      await api.get("/unit").then((response) => setUnits(response.data));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const fetchSector = async () => {
    try {
      await api.get("/sector").then((response) => setSector(response.data));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const fetchBatch = async () => {
    try {
      await api.get("/batch").then((response) => setBatch(response.data));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const fetchLocals = async () => {
    try {
      await api.get("/local").then((response) => setLocals(response.data));
    } catch (err) {
      console.log(err);
    }
  };
  ...
  const handleDelete = async (product_id) => {
    ...
    if (jsonUser.role === "admin") {
      try {
        await api.delete(`/products/${product_id}`).then((response) => {
  ...
  const handleCategoryUpdate = async (e) => {
    ...
    try {
      ...
      await api
        ...
        /**
         * TODO: Por algum motivo não está sendo passado o valor da imagem para o o estado, se dermos um console.log no response.data.category_image será mostrado o endereço certo. 
         */
        .then(response => setNewCategoryImageUrl(response.data.category_image))
  ...
  const handleCategoryDelete = async (category_id) => {
    ...
    if (jsonUser.role === "admin") {
      try {
        await api.delete(`/category/${category_id}`).then((response) => {
          console.log(response);
        });
  ...
  return (
    ...
    <div className="w-full alt-color-2-bg rounded border-[15px] border-[#6B3710] shadow-[0px_2px_2px_2px_rgba(0,0,0,0.25)] mt-4">
      <div className="border-l-[6px] border-r-[6px] border-[#D87B26] p-[1rem] h-[200px] overflow-y-auto flex flex-wrap relative">
        <div
          className={`transition-opacity duration-200 absolute inset-0 alt-color-6-bg z-10 flex flex-col items-center justify-center ${
        ...
        >
          ...
          <div className="flex justify-evenly w-[10%]">
            ...
            <p
              ...
              onClick={() => handleCategoryDelete(props.categoryKey)}
            >
        ...
        {props.products.map((product, index) => {
          const unit =
            units.find((u) => u.unit_id === product.unit_id)?.unit_type ||
            "N/A";
          return (
            <Tooltip
              key={index}
              html={
                <div className={styles.myTippyTheme}>
                  <strong>Nome:</strong> {product.product_name}
                  <br />
                  <strong>Unidade:</strong> {unit}
                  <br />
                  <strong>Perecível:</strong>{" "}
                  {product.is_perishable ? "Sim" : "Não"}
                </div>
              }
            ...
            >
              <div
                key={index}
                className={`relative w-12 h-12 mb-4 bg-transparent border-l-[3px] border-b-[3px] border-[#FFE4A1] cursor-pointer ${styles.borderDepth}`}
            ...
            <form onSubmit={handleSubmit}>
              ...
              <div className="form-control mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProductCostValue(parseFloat(e.target.value) || 0)
                  }
                />
              </div>
              ...
              <div className="form-control mb-4">
                ...
                <input
                  ...
                  onChange={(e) =>
                    setProductSellValue(parseFloat(e.target.value) || 0)
                  }
                />
              </div>
      ...
      {isProdEditModalOpen && (
        <Modal
          closeModal={closeProdEditModal}
          title="Editar Produto"
          handleSubmit={handleProdUpdate}
        >
          <div className="form-control mb-4">
            <label className="label">
              <span className="label-text text-white">Nome do produto</span>
